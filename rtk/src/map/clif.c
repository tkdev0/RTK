...
/*int encrypt(int fd)
{
	...
	{	//tk_crypt2(WFIFOP(fd,0),sd->status.EncKey);
		tk_crypt2(WFIFOP(fd,0),&(sd->status.EncKey));
...
int decrypt(int fd)
{
	...
	{
		tk_crypt2(RFIFOP(fd,0),&(sd->status.EncKey));
	} else {
